...
<div class="outer">
	...
	<!-- 페이징바 만들기 -->
		<div class="pagingArea" align="center">
			<!-- 맨 처음으로 (<<) -->
			<button onclick="location.href='<%=request.getContextPath()%>/memberList.faq?currentPage=1'"> &lt;&lt; </button>
		
			<!-- 이전페이지로(<) -->
			<%if(currentPage == 1){ %>
			<button disabled> &lt; </button>
			<%}else{ %>
			<button onclick="location.href='<%= request.getContextPath() %>/memberList.faq?currentPage=<%= currentPage-1 %>'"> &lt; </button>
			<%} %>
			...
			<%for(int p=startPage; p<=endPage; p++){ %>
				...
				<%}else{ %>
				<button onclick="location.href='<%=request.getContextPath() %>/memberList.faq?currentPage=<%= p %>'"> <%= p %> </button>
				<%} %>
			...
			<!-- 맨 끝으로 (>>) -->
			<button onclick="location.href='<%=request.getContextPath()%>/memberList.faq?currentPage=<%=maxPage%>'"> &gt;&gt; </button>
		</div> 
