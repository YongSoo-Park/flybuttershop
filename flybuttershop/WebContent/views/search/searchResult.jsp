...
function wishListAdd(pCode,pName,clickObj) {
	...
	if(confirm("["+pName + "] 상품을 위시리스트에 등록하시겠습니까?")){
		...
		/*form.setAttribute('action', 'addWish.mp');  */
	}
}
...
<body style="margin: 0 auto">
	...
	<main>
		<c:if test="${requestScope.searchListEmpty == 0}">
			<div id="searchSalePTitle">
				할인중인 <span style="font-size: 35px;">'<c:out
						value="${requestScope.sWord}" />'
				</span> 상품
			</div>
			<div id="searchSalePWrap">
				...
				<c:forEach items="${requestScope.searchSaleList}" var="list">
					<div id="searchSaleItems" class="searchSaleItems">
						<img alt=""
							src="${pageContext.request.contextPath}${list.pImage_Origin}"
							width="190px" height="190px" onclick="detailP('${list.pCode}')"
			...
			<div id="searchListPWrap">
				...
				<div id="searchListPTitle">
					<span style="font-size: 35px;">'<c:out
							value="${requestScope.sWord}" />'
					</span> 검색 결과
				</div>
				<div>
					<select
						onchange="listPageMove(1,'${requestScope.sWord}',${requestScope.paging.total},this.value)"
						id="selectSKind">
				...
				<c:forEach items="${requestScope.searchList}" var="list">
					<div class="searchList">
						<div class="searchListItem1">
							<img alt=""
								src="${pageContext.request.contextPath}${list.pImage_Origin}"
								width="130px" height="130px" onclick="detailP('${list.pCode}')"
								style="cursor: pointer;">
						</div>
						<div class="searchListItem2">
							<br> <span style="font-size: 23px; font-weight: bold;"><a
								onclick="detailP('${list.pCode}')" style="cursor: pointer;"><c:out
										value="${list.pName}" /></a></span><br>
							<br>
							<c:out value="${list.price}원" />
							<br>
							<br> <span style="font-size: 13px;"><c:out
									value="평점 : ${list.score_Avg} 점" /> <c:out
									value="재고 : ${list.pStock}" /> </span>
						</div>
						<div class="searchListItem3">
							<c:choose>
								<c:when test="${list.wishList == 1}">
									<img
										src="${pageContext.request.contextPath}/resources/icon/favHeart.png"
										style="margin: 10px" />
								</c:when>
								<c:otherwise>
									<img
										src="${pageContext.request.contextPath}/resources/icon/eHeart.jpg"
										style="margin: 10px; cursor: pointer;"
										onclick="wishListAdd('${list.pCode}','${list.pName}')" />
								</c:otherwise>
							</c:choose>
						</div>
						<div class="searchListItem4">
							<div class="searchListItem4child">
								<span
									style="font-size: 20px; font-weight: bold; line-height: 200%">
									<c:out value="${list.store_Name}" />
								</span><br>
								<c:choose>
			...
			<div id="pageNumber">
				<c:if test="${requestScope.paging.prePage == 1}">
					<a href="#"
						onclick="listPageMove(${requestScope.paging.startPage - requestScope.paging.cntPage},'${requestScope.sWord}',${requestScope.paging.total},${requestScope.sKind})"><</a>
&nbsp;
<a href="#"
						onclick="listPageMove(1,'${requestScope.sWord}',${requestScope.paging.total},${requestScope.sKind})">1</a>
&nbsp;···&nbsp;
</c:if>
				<c:forEach varStatus="status"
					begin="${requestScope.paging.startPage}"
					...
					<c:choose>
						...
						<c:otherwise>
							<a href="#"
								onclick="listPageMove(${status.index},'${requestScope.sWord}',${requestScope.paging.total},${requestScope.sKind})"><c:out
									value="${status.index}" /></a>
						</c:otherwise>
...
···&nbsp;
<a href="#"
						onclick="listPageMove(${requestScope.paging.lastPage},'${requestScope.sWord}',${requestScope.paging.total},${requestScope.sKind})"><c:out
							value="${requestScope.paging.lastPage}" /></a>
&nbsp;
<a href="#"
						onclick="listPageMove(${requestScope.paging.startPage + requestScope.paging.cntPage},'${requestScope.sWord}',${requestScope.paging.total},${requestScope.sKind})">></a>
&nbsp;
		...
		<c:if test="${requestScope.searchListEmpty == 1}">
			<div id="wrongWord">
				검색하신 단어 '
				<c:out value="${requestScope.sWord}" />
				' 에 <br>관련된 상품을 찾지 못했습니다.<br>
			</div>
